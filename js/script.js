  ...
  //console.log(selected.q);
	...
	var a11y_policies = $.getJSON( "./js/data.json", function() {
		...
		var settings = {
			...
			orderByTemplate    : '',
			countTemplate      : '<div class="facettotalcount"><strong><span aria-live="true">Showing <%= count %> <% if (count==1) { %>item<% } else {%>items<% } %></span><% if (filters) { %>, matching the filters:</strong> <span class="filter"><%= filters.join("</span>, <span class=\'filter\'>") %></span><% } else { %></strong><% } %></div>',
